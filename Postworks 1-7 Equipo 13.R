...
# Entonces, ¿el resultado de un partido es influenciado por quién juega de local 
# y de visitante?
...
DFmargLocal %>%
  ggplot() + 
  aes(x = Var1, y = Freq) +
  geom_bar( col = "black", fill = "plum3", stat = "identity") + 
...
# Estas observaciones son similares a aquellas en el primer postwork. Entonces, 
# tomando la hipótesis planteada anteriormente como la hipótesis alterna, 
# se determina la hipótesis nula como: "La media de goles anotados por equipos 
# locales es igual que la media de goles anotados por equipos visitantes", 
# y procederemos a aceptarla o rechazarla mediante un proceso de contraste de hipótesis.

m1 <- Ligas1720$FTHG
m2 <- Ligas1720$FTAG
n1 <- length(Ligas1720$FTHG)
n2 <- length(Ligas1720$FTAG)
z0 <- (mean(m1)-mean(m2))/sqrt(var(m1)/n1 + var(m2)/n2)
(z.05 <- qnorm(p = 0.05, lower.tail = FALSE)) #Región de rechazo
(z0 > z.05)
(pvalue <- pnorm(z0, lower.tail = FALSE)) #Cálculo del p-value

x <- seq(-8, 8, 0.01)
y <- dnorm(x)
plot(x, y, type = "l", xlab="", ylab="") 
title(main = "Densidad normal estándar") 
polygon(c(z.05, x[x>=z.05], max(x)), c(0, y[x>=z.05], 0), col="seagreen3") 
axis(side = 1, at = z0, font = 2, padj = 1, lwd = 2) 
axis(side = 1, at = z.05, font = 2, padj = 1, lwd = 2)

#Como el p-value y el valor z están dentro de la zona de rechazo, entonces rechazamos 
# la hipótesis nula, lo que significa que, en efecto, "La media de goles anotados 
# por equipos locales es mayor que la media de goles anotados por equipos visitantes", 
# es decir, es más probable que los equipos locales anoten goles durante los partidos.
...
#     Por ejemplo, cuando se tienen 8 goles por parte de un equipo, el coeficiente 
#     suele ser o muy pequeño (en el orden de 0.001) o muy grande (mayor a 2).

# Entonces, en los casos que se considera que los cocientes de la tabla del punto 1
# tienen un valor de 1, son los marcadores de 0-0, 1-0 y 0-1, e incluso se pueden 
# considerar los marcadores cercanos.
...
# Implica que los goles, ya sea del equipo local o visitante, no se hacen más o menos 
# probables en función de los goles anotados durante en el intervalo de 90 minutos, 
# es decir, lo que dura el partido. 
...
#Creación de un único data frame con la fecha en formato correcto.
...
listasoccer <- create.fbRanks.dataframes("https://raw.githubusercontent.com/AEscajeda/Programacion-y-Estadistica-con-R/main/soccer.csv")
...
# la serie de tiempo, se eliminaron los datos de Junio 2013, y los del año 2020.
promedio.mes <- promedio.mes[-c(31, 97, 98, 99, 100, 101),]
...
#Consulta del número de goles que metió el Real Madrid el 20 de diciembre de 2015
#y contra que equipo jugó. 
mongo_conn$find('{"date":"2015-12-20", "$or": [ { "home.team": "Real Madrid" },
...
# Importante comentar que es posible realizar dicha conexión con un servidor remoto 
